...
class NavigationRegistry
{
     ...
     */
    protected static function sectionDefinitions(): array
    {
        return [
            'workbench' => [
                'title' => '工作台',
    ...
    /**
     * @return array<int, array{title: string, description: string, route: string, icon: string, group: string, sort: int, permission?: string, show_in_sidebar?: bool, show_in_dashboard?: bool, dashboard_description?: string}>
     */
    ...
    {
        $itemsByGroup = collect(self::definitions())
            ->filter(fn (array $item): bool => ($item['show_in_sidebar'] ?? true) && self::canAccess($item, $user))
            ->sortBy('sort')
            ->groupBy('group');

        return collect(self::sectionDefinitions())
            ->sortBy('sort')
            ->map(function (array $section, string $group) use ($itemsByGroup): array {
                return [
                    'title' => $section['title'],
                    'items' => collect($itemsByGroup->get($group, []))
