...
4. 将 `components.json` 锁定为 JavaScript 模式：
    - `typescript: false`
    - `style: "new-york"`
...
- `resources/js/lib/ui-resolver.js`
...
- 不开放注册：
    ...
    - 前端不提供注册入口
- `App\Models\User` 实现 `MustVerifyEmail`。
- 后台创建用户后的规则：
...
- Vite 增加：
    - `vue()`
    - `Components({ dts: false, resolvers: [uiResolver()] })`
    - `resolve.alias['@'] = /resources/js`
- 新增 `jsconfig.json`，配置 `@/* -> resources/js/*`。
- 只自动导入 `Ui*` 基础组件，不自动导入业务组件。
...
- 简单表单坚持 `useForm`；复杂动态表单后续再引入 `vee-validate`。
- 参考文档：
