        ...
        <form class="grid gap-2 lg:grid-cols-[minmax(0,2fr)_repeat(2,minmax(0,1fr))_auto] lg:items-center" @submit.prevent="submit">
            <div class="relative">
                ...
                <UiInput
                    ...
                    placeholder="按命令类型、客户端ID、用户名、对端地址或协议搜索"
                />
            </div>
            <UiInput v-model="form.event_type__eq" class="h-10 rounded-lg" placeholder="命令类型" />
            <UiInput v-model="form.protocol__eq" class="h-10 rounded-lg" placeholder="协议" />
